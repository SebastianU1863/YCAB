...
There are three Log Organizers: Application, Error, and Webserver. All Log Organizers output csv files. Pak Erwin provided samples and example structures.
  ...
  The Application Log Organizer organizes Pak Erwin's application log example from the "Struktur Log untuk Application & DB" file.
  The Error Log Organizer organizes errors like the ones found in the "single-laravel" file. This uses RegEx to match patterns.
  The Webserver Log Organizer organizes the webserver logs which include various user data like the device name or ip address.
...
I have turned my program into an executable. If you are using this program here are the steps:
  1. Download the "budget-compiler" folder
  2. Look at the templates provided, Budget Template and Payment Template are the respective templates for how your budget and payment spreadsheets/csv files should be formatted
  ...
  3. Run the executable (.exe file) found in the dist folder. If you have downloaded the build and have everything, the code will show a popup with instructions.
